#!/usr/bin/env python3
"""
Первый Пруд 3.0 — С УБЕЖИЩАМИ
Дафнии под водорослями НЕДОСТУПНЫ для жуков
"""
...
class Daphnia:
    ...
    def pod_zashitoy(self, world):
        """Проверяет, стоит ли дафния на водоросли (в безопасности)"""
        for a in world.algae:
...
class Beetle:
    ...
    def update(self, world):
        ...
        # Охота — ищем дафнию НЕ под защитой
        лучшая_дафния = None
        лучшее_расстояние = 1000

        for d in world.daphnia:
            # ВАЖНО: жук НЕ видит дафний, которые под водорослями
            if d.pod_zashitoy(world):
                continue  # Пропускаем защищённых
...
def draw(stdscr, world):
    ...
    # Статистика
    a, d, b, stealth, neuro, protected = world.stats()
    ...
    if a == 0:
        ...
        state = "🌊 ЖУКИ ВЫМЕРЛИ"
    elif protected > d * 0.3:
        state = "🛡️ МНОГО ПОД ЗАЩИТОЙ!"
    elif neuro > 0.6:
        state = "🧠 НЕЙРОТИЧНЫЕ"
    else:
        state = "⚖️ ЦИКЛ"

    status = f"A:{a:3} D:{d:3} B:{b:3} | Stl:{stealth:.2f} Neuro:{neuro:.2f} | Защита:{protected} | {state}"
